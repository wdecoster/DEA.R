...
sanityCheck <- function(sample_info, annotation, outputdir) {
    inputdata <- list()
    if (! dir.exists(outputdir)) { dir.create(outputdir)}
    inputdata$outdir <- outputdir
...
getCountsFromBam <- function(inputdata) {
    ...
    capture.output(
        ...
        file = file.path(inputdata$outdir, "FeatureCounts.log"))
    ...
    write.table(
        ...
        file=file.path(inputdata$outdir, "FeatureCounts_RawCounts.txt"),
        quote=F,
        sep="\t",
        row.names=F)
    countStats(
    ...
    counts <- counts$counts
    save(counts, file=file.path(inputdata$outdir, "FeatureCounts_RawCounts.RData"))
    return(counts)
    }
...
countStats <- function(statdat, samples, inputdata, counts) {
    ...
    suppressMessages(ggsave(file.path(inputdata$outdir, "FeatureCounts_NoFeaturePlot.jpeg"), p))
    ...
    write.table(
        ...
        file=file.path(inputdata$outdir, "FeatureCounts_CountingStatistics.txt"),
        quote=F,
        sep="\t",
        row.names=F)
...
genderPlots <- function(inputdata, counts) {
    ...
    p <- ggplot(data = data, aes(x=f, y=m, colour=gender)) +
        ...
        suppressMessages(ggsave(file.path(inputdata$outdir, "GenderSpecificExpression.jpeg"), p))
        }
...
proc_limma_voom <- function(inputdata) {
    ...
    write.table(
        x=normalizedCounts,
        file=file.path(inputdata$outdir, "Limma-voom_normalizedcounts.txt"),
        sep="\t",
        row.names=FALSE,
        quote=FALSE)
    write.table(
        x=normalizedCounts[,c("gene", "symbol")],
        file=file.path(inputdata$outdir, "Limma-voom_consideredgenes.txt"),
        sep="\t",
        row.names=FALSE,
        quote=FALSE)
    ...
    write.table(
        ...
        file=file.path(inputdata$outdir, "Limma-voom_differential_expression.txt"),
        sep="\t",
        row.names=FALSE,
        quote=FALSE)
    write.table(
        x=as.data.frame(DEG[,c("gene", "symbol")]),
        file=file.path(inputdata$outdir, "Limma-voom_DEG.txt"),
        sep="\t",
        col.names=FALSE,
        row.names=FALSE,
        quote=FALSE)
...
exploratoryDataAnalysisDESeq <- function(dds) {
    jpeg(
        filename=file.path(inputdata$outdir, 'DESeq2_MAplot.jpeg'),
        width=8,
        height=8,
        units="in",
        res=500)
    ...
    dev.off()
    jpeg(
        filename=file.path(inputdata$outdir, 'DESeq2_Dispersionplot.jpeg'),
        width=8,
        height=8,
        units="in",
        res=500)
    ...
    write.table(
        ...
        file=file.path(inputdata$outdir, "DESeq2_vst_normalizedcounts.txt"),
        sep="\t",
        row.names=FALSE,
        quote=FALSE)
    write.table(
        x=rlddf[,c("gene", "symbol")],
        file=file.path(inputdata$outdir, "DESeq2_genesconsidered.txt"),
        row.names=FALSE,
        quote=FALSE)
    }
...
getDESeqDEAbyContrast <- function(dds, group) {
    ...
    write.table(
        x=as.data.frame(output),
        file=file.path(inputdata$outdir, paste("DESeq2", contrast, "differential_expression.txt", sep="_")),
        sep="\t",
        row.names=FALSE,
        quote=FALSE)
    ...
    write.table(
        x=DEG,
        file=file.path(inputdata$outdir, paste("DESeq2", contrast, "DEG.txt", sep="_")),
        sep="\t",
        col.names=FALSE,
        row.names=FALSE,
        quote=FALSE)
    makeVolcanoPlot(
        ...
        names=c("padj<0.1", "Not Sig"))
    jpeg(
        filename=file.path(inputdata$outdir, paste('DESeq2', contrast, 'Histogram_pvalues.jpeg', sep="_")),
        width=8,
        height=8,
        units="in",
        res=500)
...
proc_edger <- function(inputdata) {
    ...
    jpeg(
        filename=file.path(inputdata$outdir, 'edgeR_MAplot.jpeg'),
        width=8,
        height=8,
        units="in",
        res=500)
    ...
    write.table(
        x=output,
        file=file.path(inputdata$outdir, "edgeR_differential_expression.txt"),
        sep="\t",
        row.names=FALSE,
        quote=FALSE)
    write.table(
        x=as.data.frame(DEG[,c("gene","symbol")]),
        file=file.path(inputdata$outdir, "edgeR_DEG.txt"),
        sep="\t",
        col.names=FALSE,
        row.names=FALSE,
        quote=FALSE)
...
exploratoryDataAnalysisedgeR <- function(disp){
    jpeg(
        filename=file.path(inputdata$outdir, 'edgeR_dispersionPlot.jpeg'),
        width=8,
        height=8,
        units="in",
        res=500)
    ...
    write.table(
        ...
        file=file.path(inputdata$outdir, "edgeR_normalizedCounts_cpm.txt"),
        sep="\t",
        col.names=TRUE,
        row.names=FALSE,
        quote=FALSE)
    write.table(
        ...
        file=file.path(inputdata$outdir, "edgeR_genesconsidered.txt"),
        sep="\t",
        col.names=TRUE,
        row.names=FALSE,
        quote=FALSE)
...
makeHeatMap <- function(normcounts, proc, names){
    ...
    jpeg(
        filename=file.path(inputdata$outdir, paste(proc, 'SamplesHeatMap_normalizedcounts.jpeg', sep="_")),
        width=8,
        height=8,
        units="in",
        res=500)
...
makePCA <- function(normcounts, proc, sampleInfo) {
    ...
    for (cov in covariates) {
        ...
        suppressMessages(ggsave(file.path(inputdata$outdir, paste(proc, 'PCAplot', cov, 'normalizedcounts.jpeg', sep="_")), a))
        }
    }
...
makeScree <- function(pca, proc){
    ...
    suppressMessages(ggsave(file.path(inputdata$outdir, paste(proc, 'PCA_scree.jpeg', sep="_")), scree, device="jpeg"))
    }
...
makeVolcanoPlot <- function(input, toolname, names) {
    ...
    suppressMessages(ggsave(file.path(inputdata$outdir, paste(toolname, "Volcanoplot.jpeg", sep="_")), device="jpeg"))
    }


makeVennDiagram <- function(set1, set2, set3) {
    jpeg(
        filename=file.path(inputdata$outdir, "VennDiagram-DEG.jpeg"),
        width=8,
        height=8,
        units="in",
        res=500)
...
arg <- get_args()
inputdata <- sanityCheck(arg$sample_info, arg$annotation, arg$output)
